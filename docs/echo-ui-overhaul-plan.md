...
> 状态（2026-07-15）：P0 合同与 P1 foundations 已进入当前提交序列；P2-P4b 的移动端页面与组件迁移、P5 的状态与韧性清理已进入当前阶段工作区，P6 文档与纯构建收尾正在进行。M0 截图基线仍按 `docs/ui-baseline-manifest.md` 管理。
>
> 该状态只描述实现进度，不代表最终验收。新版 Android/iOS 截图、明暗模式与设备矩阵、200% 字体、减少动效、读屏、性能、完整测试与构建、最终来源审计仍待执行或汇总；不得据此宣称 UI 重构已经测试通过或全部完成。
...
| 批次 | 当前实现状态 | 尚缺验收证据 |
|---|---|---|
| P0-P1 | 产品合同、设计系统、语义 token、Echo 基础组件与图标入口已进入当前提交序列 | 完整提交序列上的最终来源审计 |
| P2-P4b | 壳层、播放器、主入口、曲库详情、设置表单与元数据编辑器迁移已进入当前阶段工作区 | 全量回归、真实设备视觉检查、动态字体与读屏验收 |
| P5 | 下载、离线、缓存、统计、加载/空内容/错误/弱网状态及反馈出口已进入当前阶段工作区 | 弱网和离线场景复核、性能检查、完整测试结果 |
| P6 | README、快速上手截图说明、UI 导出说明与本计划正在同步 | 新版截图与 UI 导出、完整构建结果及最终验收记录 |
